...
while running:
    ...
    for event in pygame.event.get():
        ...
        # Handle State 8 (Song Selection)
            ...
            button_click_delay = 10000  # Delay in milliseconds (e.g., 2000ms = 2 seconds)
    ...
    if state == 1:
        ...
        # Render the stored AI response on the screen, split into three lines
        font = pygame.font.Font(None, 24)  # Use a default font with size 36
        line1 = ai_response1[:len(ai_response1)//3]  # First third of the text
        line2 = ai_response1[len(ai_response1)//3:2*len(ai_response1)//3]  # Second third of the text
        line3 = ai_response1[2*len(ai_response1)//3:]  # Last third of the text

        # Render each line and display it on the screen
        rendered_text1 = font.render(line1, True, (255, 255, 255))  # White text color
        rendered_text2 = font.render(line2, True, (255, 255, 255))  # White text color
        rendered_text3 = font.render(line3, True, (255, 255, 255))  # White text color
        screen.blit(rendered_text1, (50, 500))  # Display the text at the bottom of the screen
        screen.blit(rendered_text2, (50, 525))  # Display the text at the bottom of the screen
        screen.blit(rendered_text3, (50, 550))  # Display the text at the bottom of the screen
        ...
        # Render the stored AI response on the screen, split into three lines
        font = pygame.font.Font(None, 24)  # Use a default font with size 36
        line1 = ai_response2[:len(ai_response2)//3]  # First third of the text
        line2 = ai_response2[len(ai_response2)//3:2*len(ai_response2)//3]  # Second third of the text
        line3 = ai_response2[2*len(ai_response2)//3:]  # Last third of the text
        ...
        # Add a button to transition to results or next state
        finish_button_rect = pygame.Rect(750, 100, 150, 50)  # Button for "Finish"
        pygame.draw.rect(screen, (255, 0, 0), finish_button_rect)  # Red button
        finish_text = font.render("Finish", True, (0, 0, 0))  # Black text
        screen.blit(finish_text, (finish_button_rect.x + 10, finish_button_rect.y + 10))
        button_click_delay = 10000  # Delay in milliseconds (e.g., 2000ms = 2 seconds)
